...
let caloriesArr = [];
const calories = (data) => {
  const numer = Number(data);
  caloriesArr.push(numer);
};
...
const createNewElf = (data) => {
  data.forEach((element) => {
    if (element > 0) {
      sum += element;
    } else {
      const addElf = new Elf(sum);
      sum = 0;
      elvesList.push(addElf);
    }
  });
};

createNewElf(caloriesArr);
// console.log(elvesList);

let arrayOfCalories = [];
elvesList.forEach((element) => {
